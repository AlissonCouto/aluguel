	...
	if($_POST){
		...
		if(!isset($msg['error'])){
			try{
				...
				$idVehicle = $vehicleDao->insert($vehicle);

				$sql->commit();

				$msg['success'] = "Veículo cadastrado com sucesso!";
...
?>
    ...
    <div class="workspace">
        <div class="container">
            <h1 class="title">Cadastrar veículo</h1>
                    ...
                    <select id="model" name="model">
                        ...
                        <?php
                            foreach($models as $model){
                                ?>
                                <option value="<?= $model->getId(); ?>"><?= $model->getBrand()->getDescription() . "  --  " . $model->getDescription(); ?></option>
                                <?
                            }
                        ?>
